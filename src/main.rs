...
struct Cli {
    #[clap(help = "Path to projects directory containing log files (defaults to ~/.claude/projects/)")]
    projects_dir: Option<PathBuf>,
    
    #[clap(short, long, default_value = "3000", help = "Port to serve on")]
    port: u16,
}
            ...
            if entry.file_type().is_dir() {
                ...
                for log_entry in WalkDir::new(entry.path()).min_depth(1).max_depth(1) {
                    ...
                    if log_entry.file_type().is_file() && 
                       log_entry.path().extension().map_or(false, |ext| ext == "jsonl") {
                        session_count += 1;
                            ...
                            for line in content.lines().take(5) {
                                if let Ok(entry) = serde_json::from_str::<LogEntry>(line) {
                                    if let Some(timestamp) = entry.timestamp {
                                        if latest_activity.map_or(true, |latest| timestamp > latest) {
...
async fn get_projects(State(state): State<AppState>) -> Result<Json<Vec<ProjectSummary>>, StatusCode> {
    if let Err(_) = state.refresh_cache().await {
...
async fn get_session_logs(
    ...
    let log_path = state.projects_dir
        .join(&project_name)
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if !projects_dir.exists() {
        eprintln!("Projects directory does not exist: {}", projects_dir.display());
    ...
    let app = Router::new()
        ...
        .route("/api/projects/:project/sessions/:session", get(get_session_logs))
    ...
    println!("🚀 Claude Code Log Viewer running on http://localhost:{}", cli.port);
